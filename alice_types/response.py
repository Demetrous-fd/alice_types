...
from alice_types import Button, Analytics, BigImage, ImageGallery, ItemsList
    

class Response(BaseModel):
    text: str = Field(..., max_length=1024)
    tts: Optional[str] = Field(default=None, max_length=1024)
    card: Optional[Union[BigImage, ItemsList, ItemsList]] = Field(default=None)
    buttons: Optional[List[Button]] = Field(default_factory=list)
    end_session: bool = Field(default=False)
    directives: Optional[dict] = Field(default=None)
